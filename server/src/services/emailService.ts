...
export const sendNewSessionNotification = async (
  ...
  try {
    ...
    const sessionStart = formatDateTime(startTime);
    const sessionEnd = endTime.toLocaleTimeString('en-US', {
      hour: '2-digit',
      minute: '2-digit'
    });

    // Create a session dashboard URL (adjust as needed for your app)
    const sessionUrl = `${process.env.FRONTEND_URL || 'http://localhost:5173'}/admin?sessionId=${sessionId}`;
    ...
    if (isRecurring && recurrenceType) {
      ...
      const endDateText = recurrenceEndDate
        ? recurrenceEndDate.toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' })
        : 'Not specified';

      recurrenceInfo = `
        <tr style="background-color: #fef3c7;">
          <td colspan="2" style="padding: 10px; border-bottom: 1px solid #e5e7eb;">
            <strong>📅 Recurring Session:</strong> ${frequencyText} until ${endDateText}
          </td>
        </tr>
      `;
    }
    ...
    const emailHtml = `
      <div style="font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; color: #333;">
        <h2 style="color: #1f2937;">New Session Request</h2>
        <div style="padding: 15px; background-color: #dbeafe; border-left: 4px solid #3b82f6; margin-bottom: 20px;">
          <p style="margin: 0; font-weight: 500;">A new session request requires your approval!</p>
        </div>

        <table width="100%" cellpadding="10" cellspacing="0" style="border-collapse: collapse; margin: 20px 0; border: 1px solid #e5e7eb;">
          <tr style="background-color: #f9fafb;">
            <td style="border-bottom: 1px solid #e5e7eb; font-weight: bold; width: 30%;">Session Title</td>
            <td style="border-bottom: 1px solid #e5e7eb;">${sessionTitle}</td>
          </tr>
          <tr>
            <td style="border-bottom: 1px solid #e5e7eb; font-weight: bold;">Requested By</td>
            <td style="border-bottom: 1px solid #e5e7eb;">${userName} (${userEmail})</td>
          </tr>
          <tr style="background-color: #f9fafb;">
            <td style="border-bottom: 1px solid #e5e7eb; font-weight: bold;">Date & Time</td>
            <td style="border-bottom: 1px solid #e5e7eb;">${sessionStart} to ${sessionEnd}</td>
          </tr>
          ${recurrenceInfo}
          <tr>
            <td style="border-bottom: 1px solid #e5e7eb; font-weight: bold;">Description</td>
            <td style="border-bottom: 1px solid #e5e7eb;">${description || 'No description provided'}</td>
          </tr>
          <tr style="background-color: #fef9c3;">
            <td colspan="2" style="padding: 15px; text-align: center; font-weight: bold; color: #854d0e;">
        ...
        <div style="margin-top: 30px; text-align: center;">
          <a href="${sessionUrl}" style="background-color: #4f46e5; color: white; padding: 12px 30px; text-decoration: none; border-radius: 5px; display: inline-block; font-weight: 500;">Review & Approve Session</a>
        </div>

        <div style="margin-top: 30px; padding-top: 20px; border-top: 1px solid #eaeaea; font-size: 12px; color: #666;">
          <p>This is an automated notification from Hedy's Studio Scheduling App.</p>
          <p>Please review and respond to this session request at your earliest convenience.</p>
        </div>
      </div>
    `;
    ...
    const mailOptions = {
      from: `"Hedy's Studio" <${EMAIL_USER}>`,
      to: adminEmail,
      subject: `🔔 New Session Request: ${sessionTitle}`,
      html: emailHtml
    };

    // Send the email
    const info = await transporter.sendMail(mailOptions);
    console.log('New session notification email sent: ', info.messageId);
    return true;
  } catch (error) {
    console.error('Error sending new session notification email:', error);
    return false;
  }
};
